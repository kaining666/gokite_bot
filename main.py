...
dfile = os.path.abspath('discord_token.txt')

def register():
    wallet_manager = EthereumWallet()
    wallet_data = wallet_manager.generate_wallet()
    checker = ProxyChecker()
    twitter_data = DataProviderFromFile(xfile)
    discord_data = DataProviderFromFile(dfile)
    while True:
        try:
            proxy = checker.get_unique_proxy()
            twitter_token = twitter_data.get_data()
            discord_token = discord_data.get_data()
            ...
            #连接discord
            token = bot.social_link('discord')
            data = bot.submit_discord_token(token, discord_token)
